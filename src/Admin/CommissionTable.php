...
class CommissionTable extends \WP_List_Table {
	...
	public function get_views() {
		$views = array(
			...
			'due'  => '<li class="all"><a href="' . admin_url( 'admin.php?page=wcv-commissions?=commission_status=due' ) . '">' . __( 'Due', 'wc-vendors' ) . '</a></li>',
			'paid' => '<li class="all"><a href="' . admin_url( 'admin.php?page=wcv-commissions?=commission_status=paid' ) . '">' . __( 'Paid', 'wc-vendors' ) . '</a></li>',
			'void' => '<li class="all"><a href="' . admin_url( 'admin.php?page=wcv-commissions?=commission_status=void' ) . '">' . __( 'Void', 'wc-vendors' ) . '</a></li>',
		);
	...
	public function extra_tablenav( $position ) {
		...
		if ( 'top' === $position ) {
			...
			submit_button( __( 'Filter', 'wc-vendors' ), false, false, false );
			submit_button( __( 'Export Commissions', 'wc-vendors' ), false, false, false, $other_attributes );
			echo '</div>';
	...
	public function months_dropdown( $post_type ) {
		...
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value='0'><?php esc_html_e( 'Show all dates', 'wc-vendors' ); ?></option>
			<?php
	...
	public function status_dropdown( $post_type ) {
		...
		<select name="commission_status">
			<option <?php selected( $commission_status, '' ); ?> value=''><?php esc_html_e( 'Show all Statuses', 'wc-vendors' ); ?></option>
			<option <?php selected( $commission_status, 'due' ); ?> value="due"><?php esc_html_e( 'Due', 'wc-vendors' ); ?></option>
			<option <?php selected( $commission_status, 'paid' ); ?> value="paid"><?php esc_html_e( 'Paid', 'wc-vendors' ); ?></option>
			<option <?php selected( $commission_status, 'void' ); ?> value="void"><?php esc_html_e( 'Void', 'wc-vendors' ); ?></option>
		</select>
		<?php
		return true;
	}
	...
	public function vendors_dropdown( $post_type ) {
		...
		?>
		<select name="vendor">
			<option <?php selected( $vendor, '' ); ?> value=""><?php esc_html_e( 'Show all Vendors', 'wc-vendors' ); ?></option>
	...
	public function get_columns() {
		$columns = array(
			...
			'order_id'          => __( 'Order', 'wc-vendors' ),
			'product_name'      => __( 'Product Name', 'wc-vendors' ),
			'vendor_name'       => __( 'Vendor', 'wc-vendors' ),
			'commission'        => __( 'Commission', 'wc-vendors' ),
			'commission_status' => __( 'Commission Status', 'wc-vendors' ),
			'order_date'        => __( 'Order Date', 'wc-vendors' ),
			'paid_date'         => __( 'Paid Date', 'wc-vendors' ),
		);
		...
		switch ( $column_name ) {
			case 'order_id':
				...
				if ( is_object( $order ) ) {
					...
					return sprintf( '%s ' . __( 'Order Not Found', 'wc-vendors' ), '#' . $order->get_order_number() );
				}
			...
			case 'vendor_name':
				...
				if ( is_object( $vendor ) ) {
					...
					return sprintf( '%s ' . __( 'Vendor Not Found', 'wc-vendors' ), '#' . absint( $item->vendor_id ) );
				}
			...
			case 'product_name':
				...
				if ( is_object( $product ) ) {
					...
					return sprintf( '%s ' . __( 'Product Not Found', 'wc-vendors' ), '#' . absint( $item->product_id ) );
				}
			...
			case 'commission_status':
				...
				if ( 'due' === $item->commission_status ) {
					$status = '<span class="wcv-due-status">' . esc_html__( 'DUE', 'wc-vendors' ) . '</span>';
				}

				if ( 'paid' === $item->commission_status ) {
					$status = '<span class="wcv-paid-status">' . esc_html__( 'PAID', 'wc-vendors' ) . '</span>';
				}
	...
	public function get_bulk_actions() {
		...
		$actions = array(
			'pay'    => __( 'Pay Commission', 'wc-vendors' ),
			'unpaid' => __( 'Mark Due', 'wc-vendors' ),
			'paid'   => __( 'Mark Paid', 'wc-vendors' ),
			'void'   => __( 'Mark Void', 'wc-vendors' ),
			'delete' => __( 'Delete Commission', 'wc-vendors' ),
		);
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			...
			$columns['cb'] = '<label class="screen-reader-text" for="cb-select-all-' . $cb_counter . '">' . esc_html__( 'Select All', 'wc-vendors' ) . '</label>'
