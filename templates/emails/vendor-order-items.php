...
foreach ( $items as $item_id => $item ) :
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
			...
			<td class="td"
				style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;"><?php echo esc_attr( apply_filters( 'woocommerce_email_order_item_quantity', $item->get_quantity(), $item ) ); ?></td>

			<?php if ( 'both' === $totals_display || 'commission' === $totals_display ) : ?>
				<td class="td"
					style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;"><?php echo esc_attr( wc_price( WCV_Commission::get_commission_due( $order->get_id(), $product->get_id(), $vendor_id ) ) ); ?></td>
			<?php endif; ?>

			<?php if ( 'both' === $totals_display || 'product' === $totals_display ) : ?>
				<td class="td"
					style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;"><?php echo esc_attr( $order->get_formatted_line_subtotal( $item ) ); ?></td>
			<?php endif; ?>
